...
import { TransactionForm, TransactionList } from '@/components/TransactionManager'
...
import { useBlocks, useMempool, useConsensus, useAutoNetworkSetup } from '@/hooks/useBlockchain'
import { useConnectionStatus } from '@/hooks/useConnection'
import { Blocks, Wallet, Send, Pickaxe, Clock, Home, Network, RefreshCw } from 'lucide-react'

function App() {
  const [activeTab, setActiveTab] = useState('overview')
  const { blocks, loading: blocksLoading, error: blocksError, refetch: refetchBlocks } = useBlocks()
  const { loading: mempoolLoading, error: mempoolError, refetch: refetchMempool } = useMempool()
  ...
  const [networkSetupMessage, setNetworkSetupMessage] = useState<string | null>(null)
      ...
      try {
        ...
        if (result) {
          if (result.singleNode) {
            ...
            setNetworkSetupMessage(`🌐 Connected to ${result.nodeCount} node network`)
          }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <ErrorBoundary>
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">Blockchain Blocks</h2>
                  <Button 
                    variant="outline" 
                    size="sm" 
                ...
                {blocksLoading ? (
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-lg p-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="font-semibold text-red-800">Failed to load blocks</h3>
                    </div>
                    <p className="text-red-700 mb-2">{blocksError}</p>
                    <p className="text-red-600 text-sm mb-4">
                      Make sure the blockchain backend is running on {import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}
                    </p>
                    <button 
                      onClick={refetchBlocks}
          ...
          <TabsContent value="mining" className="space-y-6">
            <ErrorBoundary>
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-6">Mining Interface</h2>
                <MiningInterface />
              </div>
            </ErrorBoundary>
          </TabsContent>
          ...
          <TabsContent value="mempool" className="space-y-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Memory Pool</h2>
              <ErrorBoundary>
                {mempoolLoading ? (
                  <div className="text-center py-8">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-lg p-6">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-5 h-5 text-red-600">⚠</div>
                      <h3 className="font-semibold text-red-800">Failed to load mempool</h3>
                    </div>
          ...
          <TabsContent value="network" className="space-y-6">
            <ErrorBoundary>
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-6">Network Management</h2>
                <NetworkManager />
              </div>
            </ErrorBoundary>
          </TabsContent>
      ...
      <footer className="bg-white border-t mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center text-gray-500 text-sm">
            <p>© 2024 Blockchain Explorer. Built with React, Vite, and Tailwind CSS.</p>
            <p className="mt-2">
