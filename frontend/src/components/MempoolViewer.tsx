...
export function MempoolViewer() {
  ...
  return (
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-green-600">Total Fees</span>
            </div>
          ...
          <div className="bg-purple-50 p-4 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-purple-600">Avg Fee</span>
            </div>
        ...
        {mempool.length === 0 ? (
          ...
          <div className="space-y-3">
            <h4 className="font-semibold text-gray-700 mb-3">Pending Transactions</h4>            {mempool.map((item, index) => {
              const transactionId = item.transaction.id || `tx-${index}`
              return (
                <div key={transactionId} className="p-4 border rounded-lg hover:bg-gray-50">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline">
                        {item.transaction.id ? `${item.transaction.id.slice(0, 8)}...` : `Transaction ${index + 1}`}
                      </Badge>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-mono">{truncateAddress(item.transaction.fromAddress)}</p>
                  </div>
                  <div>
                    <p className="font-medium text-gray-600">To</p>
                    <p className="font-mono">{truncateAddress(item.transaction.toAddress)}</p>
                  </div>
                  <div>
                    <p className="font-medium text-gray-600">Time in Pool</p>
                ...
                {item.transaction.payload && (
                  <div className="mt-2">
                    <p className="font-medium text-gray-600 text-sm">Payload</p>
    ...
    return (
      <ErrorFallback 
        error={err instanceof Error ? err.message : 'An unexpected error occurred in mempool viewer'}
