...
export function BlockCard({ block, isLatest = false }: BlockCardProps) {
  try {    const formatDate = (dateString: string) => {
      return new Date(dateString).toLocaleString()
    }

    const truncateHash = (hash: string | undefined | null, length: number = 10) => {
    ...
    <Card className={isLatest ? "border-green-500 bg-green-50" : ""}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">Block #{block.hash ? block.hash.slice(0, 8) : 'N/A'}</CardTitle>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="font-mono text-xs break-all">{truncateHash(block.hash, 16)}</p>
          </div>
          <div>
            <p className="font-semibold text-gray-600">Previous Hash</p>
            <p className="font-mono text-xs break-all">{truncateHash(block.previousHash, 16)}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {block.transactions.map((tx, index) => (
                <div key={tx.id || index} className="bg-gray-50 p-2 rounded text-xs">                  <div className="flex justify-between items-center">
                    <span className="font-mono">{tx.id ? tx.id.slice(0, 8) : 'N/A'}...</span>
                    <span className="font-semibold">{tx.amount} coins</span>
                  </div>                  <div className="text-gray-500 mt-1 flex items-center gap-1">
                    {!tx.fromAddress && (
                      <Badge variant="secondary" className="text-xs px-1 py-0">⛏️ Mining</Badge>
                    )}
                    <span>
                      {tx.fromAddress ? `${tx.fromAddress.slice(0, 8)}...` : 'Coinbase'} → {tx.toAddress ? tx.toAddress.slice(0, 8) : 'N/A'}...
                    </span>
                  </div>
                </div>
              ))}
