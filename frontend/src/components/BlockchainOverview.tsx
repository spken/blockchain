...
export function BlockchainOverview() {
  ...
  const latestBlock = blockchain.chain && blockchain.chain.length > 0 ? blockchain.chain[blockchain.chain.length - 1] : null
  const totalTransactions = blockchain.chain ? blockchain.chain.reduce(
    (sum, block) => sum + (block.transactions ? block.transactions.length : 0), 
    0
  ) : 0
  ...
  return (
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-600">Chain Length</span>
            </div>            <div className="text-2xl font-bold text-gray-900 mt-1">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              {blockchain.pendingTransactions ? blockchain.pendingTransactions.length : 0}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-600">Total TXs</span>
            </div>
          ...
          {latestBlock ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Previous Hash</p>
                  <p className="font-mono text-sm bg-gray-100 p-2 rounded break-all">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm">{latestBlock.timestamp ? formatDate(latestBlock.timestamp) : 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm font-medium text-gray-600">Nonce</span>
                  <span className="text-sm font-mono">{latestBlock.nonce ? latestBlock.nonce.toLocaleString() : 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm font-medium text-gray-600">Transactions</span>
                  <Badge variant="outline">{latestBlock.transactions ? latestBlock.transactions.length : 0}</Badge>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm font-medium text-gray-600">Mining Reward</span>
    ...
    return (
      <ErrorFallback 
        error={err instanceof Error ? err.message : 'An unexpected error occurred in blockchain overview'}
