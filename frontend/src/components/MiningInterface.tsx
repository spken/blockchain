...
import { Pickaxe, Loader2, Wallet as WalletIcon, RefreshCw } from "lucide-react"
...
export function MiningInterface() {
  try {
    ...
    const [selectedMiningWallet, setSelectedMiningWallet] = useState<string>('')
    ...
    useEffect(() => {
      const fetchWallets = async () => {
        try {
          ...
          if (selectedWallet && wallets.some(w => w.publicKey === selectedWallet.publicKey)) {
  ...
  const handleMine = async () => {
    if (!selectedMiningWallet) {
      setMessage({ type: 'error', text: 'Please select a wallet to receive mining rewards' })
      return
    }
    ...
    try {
      ...
      if (consensusResult.note.includes('ersetzt')) {
        setMessage({ type: 'info', text: 'Blockchain synchronized. Starting mining...' })
      } else {
        setMessage({ type: 'info', text: 'Already synchronized. Starting mining...' })
      }
          ...
          {loadingWallets ? (
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {availableWallets.map((wallet) => (
                  <option key={wallet.publicKey} value={wallet.publicKey}>
                    {truncateAddress(wallet.publicKey)} {wallet.publicKey === selectedWallet?.publicKey ? '(Active)' : ''}
                  </option>
                ))}
              </select>
              {selectedMiningWallet && (
                <div className="p-2 bg-green-50 border border-green-200 rounded text-xs">
                  ...
                  Mining rewards will be sent to: {truncateAddress(selectedMiningWallet)}
                </div>
              )}
            </div>
          ) : (
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <p className="text-sm text-yellow-800">
                No wallets found. Please create a wallet in the Wallets tab first.
              </p>
            </div>
          )}
        </div>        {message && (
        ...
        {(mining || consensusRunning) && (
          <div className="text-center">
            <Badge variant="secondary" className="animate-pulse">
              {consensusRunning ? "Synchronizing with network..." : "Mining in progress... This may take a while"}
            </Badge>
          </div>
        )}
      </CardContent>    </Card>
