...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const [walletStore, setWalletStore] = useState<Map<string, string>>(new Map())

  const handleSetSelectedWallet = useCallback((wallet: Wallet | null) => {
    setSelectedWallet(wallet)
    if (wallet) {
      // Store the private key mapping
      setWalletStore(prev => new Map(prev.set(wallet.publicKey, wallet.privateKey)))
    }
  }, [])

  const getPrivateKey = useCallback((publicKey: string) => {
